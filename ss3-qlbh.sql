...
JOIN Phieu_Xuat PX ON CTPX.So_PX = PX.So_PX
join san_pham sp on ctpx.ma_sp = sp.ma_sp
group by month(px.ngayban), year(px.ngayban),sp.ma_sp,sp.ten_sp,sp.donvitinh ;

-- ------------------------13-----------------------------
select 
year(px.ngayban),
month(px.ngayban),
sum(ctpx.soluong * ctpx.giaban) as "Doanh thu"
FROM CT_Phieu_Xuat CTPX
JOIN Phieu_Xuat PX ON CTPX.So_PX = PX.So_PX
where (month(px.ngayban) between 1 and 7) and year(px.ngayban) = 2023
group by month(px.ngayban), year(px.ngayban) ;
-- ------------------------14-----------------------------
select px.so_px,
px.ngayban,
px.ma_nv,
...
where month(px.ngayban) = 1 or month(px.ngayban) = 7
group by px.so_px;
-- -------------------15---------------------
select px.so_px,
px.ngayban,
...
kh.ten_kh,
nv.hoten,
sum(ctpx.soluong*ctpx.giaban) as "don gia",
sp.donvitinh
from  CT_Phieu_Xuat CTPX
JOIN Phieu_Xuat PX ON CTPX.So_PX = PX.So_PX
join khach_hang kh on px.ma_kh = kh.ma_kh
...
join san_pham sp on ctpx.ma_sp = sp.ma_sp
where px.ma_kh = 1
group by px.so_px, sp.donvitinh;
...
-- --------------------18----------------------
		...
		lsp.ten_lsp
		from san_pham sp 
		join ct_phieu_xuat ctpx on ctpx.ma_sp != sp.ma_sp
		join loai_sp lsp on sp.ma_lsp = lsp.ma_lsp
		where not  sp.ma_sp in (select ctpx.ma_sp from ct_phieu_xuat ctpx)
		group by sp.ma_sp
